...
WHERE PRO.CODPROD <> 0 AND PRO.USOPROD IN ('V','R')`;
      ...
      await api
        ...
        .then((response) => {
          ...
          try {
            localStorage.setItem(
              'PedidoInfoObservacao',
              String(response.data?.observacao || '')
            );
          } catch {}
  ...
  async function GetPedidoVendaIdModal(pedido: any, cod: any) {
    ...
    // Função auxiliar para buscar da API
    const fetchFromAPI = async (apiId: number) => {
      try {
        const response = await api.get(`/api/CabecalhoPedidoVenda/${apiId}`);
    ...
    if (isMobile) {
      ...
      if (abrirViaListaNaoEnviado && pedidoSelIdLS > 0) {
        ...
        try {
          ...
          // Buscar pelo ID
          const cabecalhoPorId = cabecalho.find(item => item.id === pedidoSelIdLS);
          
          if (cabecalhoPorId) {
            console.log('Dados encontrados no IndexedDB pelo ID:', cabecalhoPorId);
            await populateFromIndexedDB(cabecalhoPorId);
            return;
          }
          
          // Se não encontrar pelo ID, tentar pelo palMPV
          const cabecalhoPorPalMPV = cabecalho.find(item => item.palMPV === pedido);
          
          if (cabecalhoPorPalMPV) {
            console.log('Dados encontrados no IndexedDB pelo palMPV:', cabecalhoPorPalMPV);
            await populateFromIndexedDB(cabecalhoPorPalMPV);
            return;
          }
          
          // Se não encontrar no IndexedDB, buscar da API
          console.log('Dados não encontrados no IndexedDB, buscando da API...');
          await fetchFromAPI(pedidoSelIdLS);
          
        } catch (error) {
          console.log('Erro ao buscar do IndexedDB, tentando API...', error);
          try {
            await fetchFromAPI(pedidoSelIdLS);
          } catch (apiError) {
            setLoading(false);
            console.log('Erro também ao buscar da API', apiError);
          }
        }
      } else {
        // Buscar pelo palMPV no IndexedDB
        try {
          const db = await openDB<PgamobileDB>('pgamobile', versao);
          const store = db.transaction('cabecalhoPedidoVenda', 'readonly').objectStore('cabecalhoPedidoVenda');
          const cabecalho = await store.getAll();
          
          const cabecalhoEscolhido = cabecalho.find(item => item.palMPV == pedido);
          
          if (cabecalhoEscolhido) {
            console.log('Dados encontrados no IndexedDB:', cabecalhoEscolhido);
            await populateFromIndexedDB(cabecalhoEscolhido);
          } else {
            console.log('Dados não encontrados no IndexedDB para palMPV:', pedido);
            setLoading(false);
          }
        } catch (error) {
          setLoading(false);
